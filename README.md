...
The functionality to target a specific gender exists, just not as a 'grouping' since the baseline genders provided in the proposal doc were either 'male' or 'female'. Sure, you can say 'one or both', but the system does a good job of weighting and selecting for a single gender at the moment and comes with other weights. Gender weight is 10 points out of a max of about 40 and is binary - they either get 10 points or they don't as it was an **important** metric. If they declined or are non-binary, the results dont weigh at all for gender - keping them normalized. Results targeting other genders still appear, but are 10 points lower. 
  - If I were to change this, I'd make a `Genders` table and add applicable options in there to be used throughout the app. I would then add a join table called `offer_target_genders` and do a many-to-many type of association to `Genders` and `Offers` on it. In the suggestion results, I would weight them based on binary matching (strict inclusion, `.includes?`) of genders using base 10 score. If the Offer's `target_genders` were `['male', 'female']` and the user was `male`, it would still assign 10 points for a 'match' as it was directly targeted. If course I would test it and change the admin views for Offers.
...
While inheritance is powerful in many situations, it also has a proclivity to make code written later more coupled to previous implementations and can introduce methods that are there to 'cover holes in the missing method implementation' than dealing it at the root - which in Rails is usually the data/schema itself due to how ActiveRecord abstracts everything. Heierarchies are data/implementation coupling in and of themselves, and I prefer my interfaces to be more flexible and extensible (horizontal, really) without having to worry about adapting the superclasses appropriately. Keeping things closer to the source just simplifies things, really. 
